...
public class MainController implements Initializable {
	...
	private void buildTreeItems(TreeItem<com.skynet.javafx.model.MenuItem> parentNode) {
		...
		for (com.skynet.javafx.model.MenuItem item : menuItems) {
			...
			if (!item.getImage().isEmpty()) {
				itemNode.setGraphic(
						new ImageView(new Image(getClass().getResourceAsStream("/images/" + item.getImage()))));
			}
	...
	private void showItemContent(com.skynet.javafx.model.MenuItem menuItem) {
		System.out.println(menuItem.getService());
		...
		System.out.println(menuItem.getValue() + ADDITIONAL_TAB_TITLE);
	...
	private ToolBar buildToolBar() {
		...
		List<com.skynet.javafx.model.MenuItem> menuItems = menuItemService.getMenuItemsByParent(1l);
		for (com.skynet.javafx.model.MenuItem item : menuItems) {
	...
	// FXMLLoader(getClass().getResource("/fxml/products.fxml"));
	...
	// try {
	// FXMLLoader loader = new
	// FXMLLoader(getClass().getResource("/fxml/categories.fxml"));
	// mainPane.setCenter(loader.load());
	// } catch (IOException e) {
	// e.printStackTrace();
	// }
